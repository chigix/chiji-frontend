...
class RequireAnnotation extends AbstractAnnotation {
    ...
    public function parse($param_str) {
        ...
        if ($file->exists()) {
            if (is_null($require_resource = ResourcesManager::getResourceByFile($file))) {
                $road = Project::getRegistered()->getMatchRoad($file->getAbsolutePath());
                if (is_null($road)) {
                    throw new ResourceNotFoundException("NO roadmap found for the resource:" . $file->getAbsolutePath());
                }
                $require_resource = ResourcesManager::getResourceByFile($file);
            }
